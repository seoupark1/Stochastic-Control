...
def plot_graphs(results,
    ...
    # tracking error
    plt.subplot(2, 1, 1)
    plt.plot(results['time'], np.rad2deg(results['attitude_tracking_error']))
    plt.xlabel('Time [s]')
    plt.ylabel('Attitude Error [deg]')
    plt.title('Nadir Pointing Attitude Tracking Error')
    plt.grid(True)
    plt.subplot(2, 1, 2)
    plt.plot(results['time'], np.rad2deg(results['omega_tracking_error']))
    plt.xlabel('Time [s]')
    ...
    plt.grid(True)
    plt.subplot(2, 1, 2)
    plt.plot(results['time'][time_range], np.rad2deg(results['omega_estimation_error'][time_range]))
    plt.xlabel('time [s]')
    plt.ylabel('Angular Velocity Error [deg/s]')
